...
create table venue_tables (
  id text primary key default gen_random_uuid()::text,
  layout_id text not null references venue_table_layouts (id) on delete cascade,
...
create table events (
  ...
  layout_id text not null references venue_table_layouts (id),
...
create table guest_constraints (
  id text primary key default gen_random_uuid()::text,
  event_id text not null references events (id) on delete cascade,
  guest_a_id text not null references guests (id) on delete cascade,
  guest_b_id text not null references guests (id) on delete cascade,
